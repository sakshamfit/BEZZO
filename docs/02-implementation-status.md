...
| Mobile app (`apps/mobile`) | PARTIALLY IMPLEMENTED | Flutter/Dart buyer app now has API-backed password/OTP sign-in, Keystore/Keychain session storage, `/me` restoration, shared refresh-on-401, local sign-out, live catalog/category search, and per-supplier offer details. The cart, checkout, and order history are still demo flows; buyer profile/onboarding, live cart/order APIs, payment handoff, and production release automation remain. |
