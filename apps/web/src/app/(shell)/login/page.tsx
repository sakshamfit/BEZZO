...
function landingFor(roles: string[]): string {
  ...
  if (roles.some((role) => role.startsWith('BUYER'))) return '/';
  if (roles.some((role) => role.startsWith('PICKER'))) return '/picker';
  if (roles.some((role) => role.startsWith('ADMIN')) || roles.includes('SUPER_ADMIN')) return '/admin';
  if (roles.some((role) => role.endsWith('AGENT'))) return '/admin';
  return '/status';
}
...
function LoginForm() {
  ...
  // Dev accounts are offered when the build flag allows it, or when the backing
  // API identifies itself as the in-memory preview (so a preview deployment is
  // explorable without leaking seeded credentials from real environments).
  const [showDemo, setShowDemo] = useState(process.env.NEXT_PUBLIC_SHOW_DEMO_CREDENTIALS === 'true');

  useEffect(() => {
    if (process.env.NEXT_PUBLIC_SHOW_DEMO_CREDENTIALS === 'true') return;
    void fetch('/api/v1/version', { headers: { accept: 'application/json' } })
      .then((response) => response.json())
      .then((payload: { success: boolean; data?: { environment?: string } }) => {
        if (payload.success && (payload.data?.environment ?? '').includes('preview')) {
          setShowDemo(true);
        }
      })
      .catch(() => undefined);
  ...
  return (
    <div className="stack" style={{ gap: 'var(--space-4)', maxWidth: 980, margin: '0 auto' }}>
      <Link href="/" aria-label="BEZZO home" style={{ alignSelf: 'center', marginTop: 'var(--space-4)' }}>
        <BrandLogo />
      </Link>
      ...
      <div className="card">
        ...
        <p className="small muted" style={{ marginTop: 'var(--space-4)', marginBottom: 0 }}>
          New to BEZZO? <Link className="link" href="/register">Register your business</Link>
        </p>
      </div>
